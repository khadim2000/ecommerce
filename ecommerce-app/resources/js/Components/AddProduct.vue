      ...
      <!-- Catégorie -->
        ...
        <div v-if="success" class="bg-green-50 border border-green-200 rounded-lg p-4">
          <div class="flex items-center">
            ...
            <span class="text-green-700 font-medium">Produit ajouté avec succès !</span>
          </div>
      </div>
...
const form = ref({
  ...
  price: null,
  stock: null,
  size_ids: [],       // tableau des tailles sélectionnées
  color_ids: [],      // tableau d'IDs des couleurs sélectionnées
  category_id: '',
  image: '',
});
...
onMounted(async () => {
  try {
    const resCat = await axios.get('/api/category',{ withCredentials: true });
    categories.value = resCat.data;

    const resColors = await axios.get('/api/color', { withCredentials: true });
...
const submitForm = async () => {
  ...
  // Les tailles sont maintenant directement dans form.value.size_ids
  form.value.size = form.value.size_ids;

  try {
    const response = await axios.post('/admin/products', form.value, {
      headers: {
    ...
    emit('product-added', response.data.product);
